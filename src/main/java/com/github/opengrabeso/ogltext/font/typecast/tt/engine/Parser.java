...
public class Parser {
    ...
    public int advanceIP(int ip) {
        ...
        int dataCount;
        ip++;
        if (com.github.opengrabeso.ogltext.font.typecast.ot.Mnemonic.NPUSHB == instructions[prog][i]) {
            // Next byte is the data byte count
            dataCount = instructions[prog][++i];
            ip += dataCount + 1;
        } else if (com.github.opengrabeso.ogltext.font.typecast.ot.Mnemonic.NPUSHW == instructions[prog][i]) {
            // Next byte is the data word count
            dataCount = instructions[prog][++i];
            ip += dataCount*2 + 1;
        } else if (com.github.opengrabeso.ogltext.font.typecast.ot.Mnemonic.PUSHB == (instructions[prog][i] & 0xf8)) {
            dataCount = (short)((instructions[prog][i] & 0x07) + 1);
            ip += dataCount;
        } else if (com.github.opengrabeso.ogltext.font.typecast.ot.Mnemonic.PUSHW == (instructions[prog][i] & 0xf8)) {
            dataCount = (short)((instructions[prog][i] & 0x07) + 1);
            ip += dataCount*2;
        }
        return ip;
    ...
    public short getPushCount(final int ip) {
        ...
        if ((com.github.opengrabeso.ogltext.font.typecast.ot.Mnemonic.NPUSHB == instr) || (com.github.opengrabeso.ogltext.font.typecast.ot.Mnemonic.NPUSHW == instr)) {
            return instructions[ip >> 16][(ip & 0xffff) + 1];
        } else if ((com.github.opengrabeso.ogltext.font.typecast.ot.Mnemonic.PUSHB == (instr & 0xf8)) || (com.github.opengrabeso.ogltext.font.typecast.ot.Mnemonic.PUSHW == (instr & 0xf8))) {
    ...
    public int[] getPushData(final int ip) {
        ...
        if (com.github.opengrabeso.ogltext.font.typecast.ot.Mnemonic.NPUSHB == instr) {
            for (int j = 0; j < count; j++) {
                data[j] = instructions[prog][i + j + 2];
            }
        } else if (com.github.opengrabeso.ogltext.font.typecast.ot.Mnemonic.PUSHB == (instr & 0xf8)) {
            for (int j = 0; j < count; j++) {
                data[j] = instructions[prog][i + j + 1];
            }
        } else if (com.github.opengrabeso.ogltext.font.typecast.ot.Mnemonic.NPUSHW == instr) {
            for (int j = 0; j < count; j++) {
                data[j] = (instructions[prog][i + j*2 + 2] << 8) | instructions[prog][i + j*2 + 3];
            }
        } else if (com.github.opengrabeso.ogltext.font.typecast.ot.Mnemonic.PUSHW == (instr & 0xf8)) {
            for (int j = 0; j < count; j++) {
    ...
    public int handleElse(int ip) {
        while (instructions[ip >> 16][ip & 0xffff] != com.github.opengrabeso.ogltext.font.typecast.ot.Mnemonic.EIF) {
            ip = advanceIP(ip);
        }
        return ip;
    }
    ...
    public int handleIf(final boolean test, int ip) {
        if (test == false) {
            ...
            while ((instructions[ip >> 16][ip & 0xffff] != com.github.opengrabeso.ogltext.font.typecast.ot.Mnemonic.ELSE)
                    && (instructions[ip >> 16][ip & 0xffff] != com.github.opengrabeso.ogltext.font.typecast.ot.Mnemonic.EIF)) {
                ip = advanceIP(ip);
            }
        }
        return ip;
    ...
    public String toString() {
        ...
        while (ip < instructions[0].length) {
            sb.append(com.github.opengrabeso.ogltext.font.typecast.ot.Mnemonic.getMnemonic(getOpcode(ip)));
