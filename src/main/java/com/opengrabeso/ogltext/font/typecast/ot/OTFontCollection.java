...
import com.opengrabeso.ogltext.font.typecast.ot.table.DirectoryEntry;
import com.opengrabeso.ogltext.font.typecast.ot.table.TTCHeader;
import com.opengrabeso.ogltext.font.typecast.ot.table.Table;
import com.opengrabeso.ogltext.font.typecast.ot.mac.ResourceHeader;
import com.opengrabeso.ogltext.font.typecast.ot.mac.ResourceMap;
import com.opengrabeso.ogltext.font.typecast.ot.mac.ResourceReference;
import com.opengrabeso.ogltext.font.typecast.ot.mac.ResourceType;
...
public class OTFontCollection {
    ...
    private com.opengrabeso.ogltext.font.typecast.ot.table.TTCHeader _ttcHeader;
    private com.opengrabeso.ogltext.font.typecast.ot.OTFont[] _fonts;
    private final ArrayList<com.opengrabeso.ogltext.font.typecast.ot.table.Table> _tables = new ArrayList<com.opengrabeso.ogltext.font.typecast.ot.table.Table>();
    ...
    public com.opengrabeso.ogltext.font.typecast.ot.OTFont getFont(final int i) {
        return _fonts[i];
    }
    ...
    public com.opengrabeso.ogltext.font.typecast.ot.table.TTCHeader getTtcHeader() {
        return _ttcHeader;
    }

    public com.opengrabeso.ogltext.font.typecast.ot.table.Table getTable(final DirectoryEntry de) {
        for (int i = 0; i < _tables.size(); i++) {
            final com.opengrabeso.ogltext.font.typecast.ot.table.Table table = _tables.get(i);
    ...
    private void readImpl(final InputStream bis, final int streamLen) throws IOException {
        ...
        if (_resourceFork || _pathName.endsWith(".dfont")) {
            ...
            // Load the font data
            _fonts = new com.opengrabeso.ogltext.font.typecast.ot.OTFont[resourceType.getCount()];
            for (int i = 0; i < resourceType.getCount(); i++) {
                final ResourceReference resourceReference = resourceType.getReference(i);
                _fonts[i] = new com.opengrabeso.ogltext.font.typecast.ot.OTFont(this);
                final int offset = resourceHeader.getDataOffset() +
                        resourceReference.getDataOffset() + 4;
                _fonts[i].read(dis, offset, offset);
            }

        } else if (com.opengrabeso.ogltext.font.typecast.ot.table.TTCHeader.isTTC(dis)) {

            // This is a TrueType font collection
            dis.reset();
            _ttcHeader = new TTCHeader(dis);
            _fonts = new com.opengrabeso.ogltext.font.typecast.ot.OTFont[_ttcHeader.getDirectoryCount()];
            ...
            _fonts = new com.opengrabeso.ogltext.font.typecast.ot.OTFont[1];
