...
public class Disassembler {
    ...
    public static int advanceIP(final short[] instructions, int ip) {
        ...
        int dataCount;
        ip++;
        if (com.opengrabeso.ogltext.font.typecast.ot.Mnemonic.NPUSHB == instructions[i]) {
            // Next byte is the data byte count
            dataCount = instructions[++i];
            ip += dataCount + 1;
        } else if (com.opengrabeso.ogltext.font.typecast.ot.Mnemonic.NPUSHW == instructions[i]) {
            // Next byte is the data word count
            dataCount = instructions[++i];
            ip += dataCount*2 + 1;
        } else if (com.opengrabeso.ogltext.font.typecast.ot.Mnemonic.PUSHB == (instructions[i] & 0xf8)) {
            dataCount = (short)((instructions[i] & 0x07) + 1);
            ip += dataCount;
        } else if (com.opengrabeso.ogltext.font.typecast.ot.Mnemonic.PUSHW == (instructions[i] & 0xf8)) {
            dataCount = (short)((instructions[i] & 0x07) + 1);
            ip += dataCount*2;
        }
        return ip;
    ...
    public static int[] getPushData(final short[] instructions, final int ip) {
     ...
     public static String disassemble(final short[] instructions, final int leadingSpaces) {
        ...
        while (ip < instructions.length) {
            ...
            sb.append(ip).append(": ");
            sb.append(com.opengrabeso.ogltext.font.typecast.ot.Mnemonic.getMnemonic(instructions[ip]));
            if (getPushCount(instructions, ip) > 0) {
                final int[] data = getPushData(instructions, ip);
                for(int j = 0; j < data.length; j++) {
                    if ((instructions[ip] == com.opengrabeso.ogltext.font.typecast.ot.Mnemonic.PUSHW) ||
